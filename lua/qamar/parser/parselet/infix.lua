...
local MT = {
	...
	__tostring = function(self)
		if config.expression_display_mode == config.expression_display_modes.infix then
			local ret = {}
			local idx = 0
			local paren = self.left.precedence < self.precedence
				or self.left.precedence == self.precedence and self.right_associative
			if paren then
				idx = idx + 1
				ret[idx] = "("
			end
			...
			paren = self.right.precedence < self.precedence
				or self.right.precedence == self.precedence and not self.right_associative
			if paren then
				idx = idx + 1
				ret[idx] = "("
			end
...
return function(self, parser, left, tok)
	...
	local ret = N(
		token_node_mapping[tok.type],
		range(left.pos.left, right.pos.right),
		self.precedence,

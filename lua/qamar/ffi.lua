...
const char *lexer_take(qamar_lexer_t *, size_t *);
	...
	local qamar_token_tp = ffi.typeof("qamar_token_t*")
	...
	local size_tp = ffi.typeof("size_t*")
	...
	function lexer.new(s)
		...
		local lexer = ffi.cast(qamar_lexer_tp, ffi.C.malloc(size))
		if lexer == nil then
			return
		end
		local ret = ffi.C.lexer_new(lexer, s, len)
		if ret == 0 then
			ffi.copy(ffi.cast(void_tp, lexer.data), s, len)
			io.stdout:write("OLE!!!\n")
			io.stdout:flush()
			return ffi.gc(lexer, finalizer)
		end
		ffi.C.free(ffi.cast("void*", lexer))
	end
